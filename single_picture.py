...
# Paper style
# -------------------------
def set_paper_style(font="Times New Roman", base_fontsize=9):
    # ICWS/IEEE-like: clean serif, thin axes, subtle grid, readable at column width.
    plt.rcParams.update({
        "font.family": "serif",
...
# -------------------------
# ICWS/IEEE-friendly palette (Okabe–Ito, colorblind-safe)
# Ref: Okabe & Ito palette is widely used for publication-quality, colorblind-friendly figures.
# -------------------------
OKABE_ITO = {
    "blue": "#0072B2",
    "orange": "#E69F00",
    "green": "#009E73",
    "amber": "#F5C710",   # darker yellow for white background
    "sky": "#56B4E9",
    "vermillion": "#D55E00",
    "purple": "#CC79A7",
    "gray": "#999999",
...
# Semantic colors used in this script
COLORS = {
    "train": OKABE_ITO["blue"],
    "val": OKABE_ITO["vermillion"],
    "loss": OKABE_ITO["black"],
    "ppl": OKABE_ITO["purple"],
    "hot": OKABE_ITO["green"],
    "cold": OKABE_ITO["orange"],
    "http": OKABE_ITO["sky"],
}
...
def plot_grad_mode_fractions(df, out_dir, x="step", col=1, smooth=10, dpi=600, fmt=("pdf", "png")):
    ...
    labels = [label_map.get(c, c) for c in cols]
        ...
        stack_colors = [COLORS['hot'], COLORS['cold'], COLORS['http']]
...
def plot_cost_breakdown(df, out_dir, x="step", col=1, smooth=10, dpi=600, fmt=("pdf", "png")):
    ...
    fig, ax = plt.subplots(figsize=fig_size(col))
        ...
        # Deterministic, paper-friendly colors (avoid too-light pastels)
    pal = [OKABE_ITO['sky'], OKABE_ITO['blue'], OKABE_ITO['green'], OKABE_ITO['orange'], OKABE_ITO['vermillion'], OKABE_ITO['purple']]
    ax.stackplot(xs, *ys, labels=existing_labels, colors=pal, alpha=0.85)
